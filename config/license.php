...
class LicenseManager {
    ...
    private function checkLocalLicense() {
        try {
            ...
            // First check settings table (where license is saved during activation)
            $settingsStmt = $this->conn->prepare("SELECT setting_value FROM settings WHERE setting_key = 'license_key'");
            $settingsStmt->execute();
            $settingsResult = $settingsStmt->fetch(PDO::FETCH_ASSOC);
            
            // If license key in settings doesn't match, it's invalid
            if ($settingsResult && $settingsResult['setting_value'] !== $this->license_key) {
                return ['valid' => false, 'message' => 'License key mismatch'];
            }
            
            // If license key exists in settings, consider it valid (license server already verified it)
            if ($settingsResult && !empty($settingsResult['setting_value'])) {
                // Check if licenses table exists and has the license
                ...
                // License key exists in settings but not in licenses table - still valid (was verified by server)
                return ['valid' => true, 'message' => 'License verified and active'];
            }
 ...
 * Returns false if license is invalid/deactivated, blocking important features
 */
function isLicenseValid() {
    ...
    try {
        ...
        return $license_check['valid'] ?? false;
    } catch (Exception $e) {
        error_log("License check exception: " . $e->getMessage());
