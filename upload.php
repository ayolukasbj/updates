...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
    ...
    } elseif (!$is_edit_mode && (!isset($_FILES['audio_file']) || $_FILES['audio_file']['error'] !== UPLOAD_ERR_OK)) {
        $error = 'Please select a music file to upload.';
    } elseif (!$terms_accepted) {
        $error = 'You must agree to the Terms & Conditions to upload content.';
    } else {
        ...
        try {
            ...
            if ($result) {
                ...
                // Fire song_uploaded action for automatic sync (if plugin system is loaded)
                if (class_exists('PluginLoader')) {
                    try {
                        $song_data = [
                            ...
                            'year' => $year,
                            'genre' => $genre,
                            'file_path' => $audio_file_path ?? $existing_song['file_path'] ?? '',
                        ];
                        PluginLoader::doAction('song_uploaded', $song_id, $song_data);
                    } catch (Exception $e) {
                        error_log("Error firing song_uploaded action: " . $e->getMessage());
                    }
                }
                
                // Auto-tag MP3 file with site branding (only for new uploads or when file is updated)
                if (!$is_edit_mode || !empty($audio_file_path)) {
                    try {
                        error_log("=== MP3 Auto-tagging: Starting process ===");
                        
                        // Load plugin system if not already loaded
                        if (!class_exists('PluginLoader')) {
                            error_log("MP3 Auto-tagging: Loading plugin system...");
                            $plugin_loader_path = __DIR__ . '/includes/plugin-loader.php';
                            if (file_exists($plugin_loader_path)) {
                                require_once $plugin_loader_path;
                                error_log("MP3 Auto-tagging: plugin-loader.php loaded");
                            } else {
                                error_log("MP3 Auto-tagging: ERROR - plugin-loader.php not found at: $plugin_loader_path");
                            }
                            if (file_exists(__DIR__ . '/includes/plugin-api.php')) {
                                require_once __DIR__ . '/includes/plugin-api.php';
                                error_log("MP3 Auto-tagging: plugin-api.php loaded");
                            } else {
                                error_log("MP3 Auto-tagging: ERROR - plugin-api.php not found");
                            }
                        } else {
                            error_log("MP3 Auto-tagging: PluginLoader class already exists");
                        }
                        
                        // Initialize plugin system
                        if (class_exists('PluginLoader')) {
                            error_log("MP3 Auto-tagging: Initializing plugin system...");
                            PluginLoader::init();
                            error_log("MP3 Auto-tagging: Plugin system initialized");
                        } else {
                            error_log("MP3 Auto-tagging: ERROR - PluginLoader class not found after loading");
                        }
                        ...
                        if (class_exists('PluginLoader')) {
                            error_log("MP3 Auto-tagging: Getting active plugins...");
                            $active_plugins = PluginLoader::getActivePlugins();
                            error_log("MP3 Auto-tagging: Found " . count($active_plugins) . " active plugins");
                            
                            foreach ($active_plugins as $plugin_file) {
                                error_log("MP3 Auto-tagging: Checking plugin: $plugin_file");
                                if (strpos($plugin_file, 'mp3-tagger') !== false) {
                                    error_log("MP3 Auto-tagging: MP3 Tagger plugin found: $plugin_file");
                                    // Plugin is active, use plugin's AutoTagger
                                    $plugin_dir = dirname($plugin_file);
                                    error_log("MP3 Auto-tagging: Plugin directory: $plugin_dir");
                                    ...
                                    $auto_tagger_paths = [
                                        $plugin_dir . '/includes/class-auto-tagger.php',
                                        $plugin_dir_normalized . '/includes/class-auto-tagger.php',
                                        realpath($plugin_dir . '/includes/class-auto-tagger.php'),
                                        __DIR__ . '/plugins/mp3-tagger/includes/class-auto-tagger.php'
                                    ];
                                    
                                    error_log("MP3 Auto-tagging: Trying to load AutoTagger class...");
                                    foreach ($auto_tagger_paths as $auto_tagger_file) {
                                        error_log("MP3 Auto-tagging: Checking path: " . ($auto_tagger_file ?: 'null'));
                                        if ($auto_tagger_file && file_exists($auto_tagger_file)) {
                                            error_log("MP3 Auto-tagging: Found AutoTagger file: $auto_tagger_file");
                                            // Also load MP3Tagger class which AutoTagger depends on
                                            $mp3_tagger_file = str_replace('class-auto-tagger.php', 'class-mp3-tagger.php', $auto_tagger_file);
                                            error_log("MP3 Auto-tagging: MP3Tagger file path: $mp3_tagger_file");
                                            if (file_exists($mp3_tagger_file)) {
                                                require_once $mp3_tagger_file;
                                                error_log("MP3 Auto-tagging: MP3Tagger class loaded");
                                            } else {
                                                error_log("MP3 Auto-tagging: WARNING - MP3Tagger file not found: $mp3_tagger_file");
                                            }
                                            require_once $auto_tagger_file;
                                            $auto_tagger_loaded = true;
                                            error_log("MP3 Auto-tagging: SUCCESS - AutoTagger class loaded from: $auto_tagger_file");
                                            break;
                                        } else {
                                            error_log("MP3 Auto-tagging: File not found: " . ($auto_tagger_file ?: 'null'));
                                        }
                                    }
                                    
                                    if ($auto_tagger_loaded) {
                                        break;
                                    } else {
                                        error_log("MP3 Auto-tagging: ERROR - Could not load AutoTagger class from any path");
                                    }
                                }
                            }
                            
                            if (!$auto_tagger_loaded) {
                                error_log("MP3 Auto-tagging: ERROR - MP3 Tagger plugin not found in active plugins list");
                            }
                        } else {
                            error_log("MP3 Auto-tagging: ERROR - PluginLoader class not available");
                        }
                        
                        // Only use plugin's AutoTagger - no fallback
                        if ($auto_tagger_loaded && class_exists('AutoTagger')) {
                            error_log("MP3 Auto-tagging: AutoTagger class is available, proceeding with tagging...");
                            // Get final file path (use uploaded file if available, otherwise use existing)
                            $final_file_path = !empty($audio_file_path) ? $audio_file_path : ($existing_song['file_path'] ?? '');
                            
                            // Resolve full path
                            if (!empty($final_file_path)) {
                                $full_file_path = strpos($final_file_path, '/') === 0 || strpos($final_file_path, ':\\') !== false 
                                    ? $final_file_path 
                                    : __DIR__ . '/' . ltrim($final_file_path, '/');
                                ...
                                if (file_exists($full_file_path)) {
                                    ...
                                    // Auto-tag the file
                                    error_log("MP3 Auto-tagging: Calling AutoTagger::tagUploadedSong()");
                                    error_log("MP3 Auto-tagging: File path: $full_file_path");
                                    error_log("MP3 Auto-tagging: Song data: " . json_encode($tag_song_data));
                                    error_log("MP3 Auto-tagging: Uploader: $logged_in_user_name");
                                    
                                    $tag_result = AutoTagger::tagUploadedSong($full_file_path, $tag_song_data, $logged_in_user_name);
                                    
                                    error_log("MP3 Auto-tagging: Tag result: " . json_encode($tag_result));
                                    
                                    if ($tag_result['success']) {
                                        error_log("MP3 Auto-tagging: SUCCESS - Auto-tagged song file: $full_file_path");
                                        ...
                                        if (!empty($tag_result['new_file_path'])) {
                                            try {
                                                ...
                                                $update_file_stmt->execute([$tag_result['new_file_path'], $song_id]);
                                                error_log("Updated file path in database: " . $tag_result['new_file_path']);
                                            } catch (Exception $e) {
                                                error_log("Failed to update file path in database: " . $e->getMessage());
                                            }
                                        }
                                    }
                                }
                        ...
                        error_log("Auto-tagging error (non-fatal): " . $e->getMessage());
                    }
                }
...
<body>
            ...
            <div class="upload-form">
                ...
                <form method="POST" enctype="multipart/form-data" id="uploadForm">
                    ...
                    <div class="form-group" style="margin-bottom: 25px;">
                        <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #333;">Select music file: <span style="color: red;">*</span></label>
                        ...
                        <input type="file" id="audio_file" name="audio_file" accept="audio/*" required style="display: none;" onchange="document.getElementById('file-name').textContent = this.files[0]?.name || 'No file chosen'; validateAudioFile()">
                        <div id="file-name" style="margin-top: 5px; font-size: 12px; color: #666;">No file chosen</div>
                        <div id="audio-file-error" style="margin-top: 5px; font-size: 12px; color: #dc3545; display: none;">Please select a music file</div>
                    ...
                    <div style="border-top: 1px solid #e0e0e0; padding-top: 20px; margin-top: 20px;">
                        <div class="form-group" style="margin-bottom: 20px;">
                            <label for="title" style="display: block; margin-bottom: 8px; font-weight: 500; color: #333;">Song title: <span style="color: red;">*</span></label>
                            ...
                            <div id="title-error" style="margin-top: 5px; font-size: 12px; color: #dc3545; display: none;">Song title is required</div>
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 20px;">
                            <label style="display: flex; align-items: center; gap: 8px; font-weight: 500; color: #333; cursor: pointer;">
                                ...
                                I agree to content provider <a href="#" style="color: #ff6b35;">Terms&Conditions</a> <span style="color: red;">*</span>
                            </label>
                            <div id="terms-error" style="margin-top: 5px; font-size: 12px; color: #dc3545; display: none;">You must agree to the Terms & Conditions</div>
                    </div>
    ...
    <script>
        // Validate audio file on change
        function validateAudioFile() {
            const audioFile = document.getElementById('audio_file');
            const errorDiv = document.getElementById('audio-file-error');
        ...
        document.getElementById('uploadForm').addEventListener('submit', function(e) {
            ...
            <?php if (!$is_edit_mode): ?>
            // For new uploads: audio file, song title, production year, producer, composer, track type, track genre, terms
            const requiredFields = [
                {id: 'audio_file', name: 'Audio file', type: 'file'},
                {id: 'title', name: 'Song title', type: 'text'},
                {id: 'year', name: 'Production year', type: 'select'},
                {id: 'producer', selectId: 'producer_select', name: 'Producer', type: 'select'},
                {id: 'composer', selectId: 'composer_select', name: 'Composer', type: 'select'},
                {id: 'track_type', name: 'Track type', type: 'select'},
                {id: 'genre', name: 'Track genre', type: 'select'},
                {id: 'terms', name: 'Terms & Conditions', type: 'checkbox'}
            ];
            <?php else: ?>
            ...
            requiredFields.forEach(field => {
                ...
                if (field.selectId) {
                    ...
                    // Checkbox field
                    if (fieldElement) {
                        value = fieldElement.checked ? 'checked' : '';
                    }
