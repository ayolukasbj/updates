...
try {
    ...
    if ($conn) {
        ...
        if ($checkSongs->rowCount() > 0) {
            // All tabs show most downloaded songs (filtered by downloads count)
            $popularQuery = "
                SELECT s.*, 
                       s.uploaded_by,
            ...
            try {
                $stmt = $conn->query($popularQuery);
                $popular_today = $stmt->fetchAll(PDO::FETCH_ASSOC);
                $popular_week = $popular_today; // Same for all tabs
                $popular_month = $popular_today; // Same for all tabs
                error_log("Most Popular Section: Found " . count($popular_today) . " popular songs");
            } catch (Exception $e) {
                error_log("Most Popular Section Error: " . $e->getMessage());
            }
        }
    }
} catch (Exception $e) {
