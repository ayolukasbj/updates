    ...
    <div class="header-container">
        ...
        <a href="index.php" class="logo">
            ...
            if (!empty($site_logo)): 
                ...
                // Debug log
                error_log("Original logo from DB: " . $site_logo);
                error_log("Normalized logo: " . $normalizedLogo);
                error_log("Logo path after asset_path: " . $logoPath);
                
                // Always show logo if path exists - let browser handle loading errors
                if (!empty($logoPath) && $logoPath !== '/' && $logoPath !== $currentBaseUrl):
            ?>
                <img src="<?php echo htmlspecialchars($logoPath); ?>" 
                     alt="<?php echo htmlspecialchars($site_name ?: SITE_NAME); ?>" 
                     style="width: 32px; height: 32px; object-fit: contain; display: block;"
                     onerror="console.error('Logo failed to load:', this.src); this.style.display='none'; if(this.nextElementSibling) this.nextElementSibling.style.display='block';">
