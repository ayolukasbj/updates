...
// Updates folder path
define('UPDATES_FOLDER', 'C:\\Users\\HYLINK\\Desktop\\music - Copy\\updates');
...
function syncFileToUpdates($file_path) {
    ...
    if (!is_dir($updates_folder)) {
        if (!mkdir($updates_folder, 0755, true)) {
            return 'Cannot create updates folder: ' . $updates_folder;
        }
    }
...
function syncDirectoryToUpdates($dir_path = '.') {
    ...
    foreach ($iterator as $file) {
        ...
        // Flush output every 100 files to prevent timeout
