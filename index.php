...
// Get featured songs for sidebar
$featured_songs = [];
try {
...
// Get recent songs - use exact same query as songs.php
try {
    if ($conn) {
        $stmt = $conn->prepare("
            SELECT s.*, 
...
<head>
    ...
    <style>
        * {
        ...
        /* Songs Recently Added Grid - Mobile: 2 columns, Desktop: 6 columns (6 items total) */
        .songs-recently-added-grid {
            display: grid;
            grid-template-columns: repeat(2, 1fr);
            gap: 15px;
...
<body>
    ...
    <div class="container-custom">
        ...
        <!-- Recent News Section with Recently Uploaded Songs Sidebar -->
        ...
        <div class="recent-news-wrapper" style="margin: 40px 0; max-width: 1400px; margin-left: auto; margin-right: auto; padding: 0 15px;">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 30px; align-items: start;">
                ...
                <!-- Right: Recently Uploaded Songs Sidebar -->
                <div class="recently-uploaded-sidebar">
                    ...
                    <div style="background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden; padding: 15px;">
                        ...
                        try {
                            ...
                            error_log("Sidebar Recent Songs Error: " . $e->getMessage());
                        }
                        ?>
                        <?php if (!empty($sidebar_recent_songs)): ?>
                            <?php foreach ($sidebar_recent_songs as $index => $song): 
                                // Generate song slug
                            ...
                            <div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: <?php echo $index < count($sidebar_recent_songs) - 1 ? '1px solid #e0e0e0' : 'none'; ?>;">
                                <a href="<?php echo $songUrl; ?>" style="text-decoration: none; color: inherit; display: block;">
                                    ...
                                    <p style="font-size: 11px; color: #999; margin: 0;">
                                        ...
                                        <i class="fas fa-download"></i> <?php echo number_format($song['downloads'] ?? 0); ?>
                                    </p>
                                </a>
                            </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div style="color: #999; font-size: 14px; text-align: center; padding: 20px;">No recently uploaded songs yet.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div style="margin: 40px 0;">
            ...
            <div class="trending-songs-grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
