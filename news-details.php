...
// Check if user profile columns exist
$has_user_bio = false;
$has_user_avatar = false;
$has_user_social_links = false;
try {
    ...
    $has_user_bio = in_array('bio', $columns_lower);
    $has_user_avatar = in_array('avatar', $columns_lower);
    $has_user_social_links = in_array('social_links', $columns_lower);
} catch (Exception $e) {
    error_log('Error checking users table columns: ' . $e->getMessage());
    // Continue without these columns
}

// Build user fields for query
$user_fields = "COALESCE(u.username, 'Unknown') as author";
if ($has_user_bio) {
    $user_fields .= ", u.bio as author_bio";
}
if ($has_user_avatar) {
    $user_fields .= ", u.avatar as author_avatar";
}
if ($has_user_social_links) {
    $user_fields .= ", u.social_links as author_social_links";
}

// Fetch news article
try {
    if (!empty($news_slug)) {
        // Build query based on whether featured_image column exists and user profile columns
        if ($has_featured_image) {
            $stmt = $conn->prepare("
                SELECT n.*, $user_fields,
    ...
    // Process author profile data
    $author_profile = [
        'name' => $news_item['author'] ?? 'Unknown',
        'bio' => $news_item['author_bio'] ?? '',
        'avatar' => $news_item['author_avatar'] ?? '',
        'social_links' => []
    ];
    
    // Decode social links JSON if available
    if (!empty($news_item['author_social_links'])) {
        if (is_string($news_item['author_social_links'])) {
            $decoded_links = json_decode($news_item['author_social_links'], true);
            if (is_array($decoded_links)) {
                $author_profile['social_links'] = $decoded_links;
            }
        } elseif (is_array($news_item['author_social_links'])) {
            $author_profile['social_links'] = $news_item['author_social_links'];
        }
    }
    
    // Always fetch complete author profile data if author_id exists to ensure we get avatar
    if (!empty($news_item['author_id'])) {
        try {
            $author_fields = ['username'];
            if ($has_user_bio) $author_fields[] = 'bio';
            if ($has_user_avatar) $author_fields[] = 'avatar';
            if ($has_user_social_links) $author_fields[] = 'social_links';
            
            $authorStmt = $conn->prepare("
                SELECT " . implode(', ', $author_fields) . " 
                FROM users 
                WHERE id = ? 
                LIMIT 1
            ");
            $authorStmt->execute([$news_item['author_id']]);
            $author_data = $authorStmt->fetch(PDO::FETCH_ASSOC);
            
            if ($author_data) {
                // Always update with fetched values to ensure we have the latest data
                if (!empty($author_data['username'])) {
                    $author_profile['name'] = $author_data['username'];
                }
                if ($has_user_bio && isset($author_data['bio'])) {
                    $author_profile['bio'] = $author_data['bio'] ?? '';
                }
                // Always update avatar if column exists and data is available
                if ($has_user_avatar && isset($author_data['avatar'])) {
                    $author_profile['avatar'] = !empty($author_data['avatar']) ? trim($author_data['avatar']) : '';
                }
                
                // Decode social links
                if ($has_user_social_links && !empty($author_data['social_links'])) {
                    if (is_string($author_data['social_links'])) {
                        $decoded_links = json_decode($author_data['social_links'], true);
                        if (is_array($decoded_links)) {
                            $author_profile['social_links'] = $decoded_links;
                        }
                    } elseif (is_array($author_data['social_links'])) {
                        $author_profile['social_links'] = $author_data['social_links'];
                    }
                }
            }
        } catch (Exception $e) {
...
// Initialize author_profile with defaults if not set
if (!isset($author_profile)) {
    $author_profile = [
        'name' => $news_item['author'] ?? 'Unknown',
        'bio' => '',
        'avatar' => '',
                ...
                <div class="author-box">
                    <div class="author-box-content">
                        <div class="author-avatar">
                            <?php if (!empty($author_profile['avatar'])): ?>
                                <img src="<?php echo htmlspecialchars(asset_path($author_profile['avatar'])); ?>" alt="<?php echo htmlspecialchars($author_profile['name']); ?>" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                        ...
                        <div class="author-info">
                            <h3><?php echo htmlspecialchars($author_profile['name']); ?></h3>
                            <?php if (!empty($author_profile['bio'])): ?>
                                <p><?php echo nl2br(htmlspecialchars($author_profile['bio'])); ?></p>
                            <?php else: ?>
                                <p style="color: #999; font-style: italic;">No biography available.</p>
                            <?php endif; ?>
                            <div class="author-social">
                                <?php 
                                $social_links = $author_profile['social_links'] ?? [];
                                if (!empty($social_links['website'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['website']); ?>" target="_blank" title="Website"><i class="fas fa-globe"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['facebook'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['facebook']); ?>" target="_blank" title="Facebook"><i class="fab fa-facebook-f"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['twitter'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['twitter']); ?>" target="_blank" title="Twitter"><i class="fab fa-twitter"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['linkedin'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['linkedin']); ?>" target="_blank" title="LinkedIn"><i class="fab fa-linkedin-in"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['instagram'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['instagram']); ?>" target="_blank" title="Instagram"><i class="fab fa-instagram"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['youtube'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['youtube']); ?>" target="_blank" title="YouTube"><i class="fab fa-youtube"></i></a>
                                <?php endif; ?>
                                <?php if (!empty($social_links['spotify'])): ?>
                                    <a href="<?php echo htmlspecialchars($social_links['spotify']); ?>" target="_blank" title="Spotify"><i class="fab fa-spotify"></i></a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
