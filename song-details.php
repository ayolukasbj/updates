...
<body>
    ...
    <div class="main-content">
        ...
        <div class="download-section">
            <a href="api/download.php?id=<?php echo (int)$song['id']; ?>" class="download-btn" id="download-btn" download>
                Download Song
            </a>
                    ...
                    <?php if (!empty($mayAlsoLike)): ?>
                        <div class="songs-grid you-may-also-like-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 20px; margin-top: 20px;">
                            ...
                            foreach ($mayAlsoLike as $similarSong): 
                                ...
                                ?>
                                <div class="song-card" onclick="window.location.href='/song/<?php echo urlencode($similarSongSlug); ?>'" style="cursor: pointer; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); transition: all 0.3s;" onmouseover="this.style.transform='translateY(-3px)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 8px rgba(0,0,0,0.05)';">
                                    <div class="song-card-image" style="height: 150px; overflow: hidden; background: linear-gradient(135deg, #667eea, #764ba2); position: relative; display: flex; align-items: center; justify-content: center;">
                                        <?php if (!empty($similarCoverArt) && $similarCoverArt !== 'assets/images/default-avatar.svg'): ?>
                                        <img src="<?php echo htmlspecialchars($similarCoverArt); ?>" 
                                             alt="<?php echo htmlspecialchars($similarSong['title']); ?>" 
                                             onerror="this.style.display='none'; this.parentElement.innerHTML='<i class=\'fas fa-music\' style=\'font-size: 48px; color: white;\'></i>';"
                                             style="width: 100%; height: 100%; object-fit: cover;">
                                        <?php else: ?>
                                        <i class="fas fa-music" style="font-size: 48px; color: white;"></i>
                                        <?php endif; ?>
                                        <button class="song-card-play-btn" onclick="event.stopPropagation(); playSongCard(this)" style="position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.7); color: white; border: none; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 10;">
                                            <i class="fas fa-play"></i>
                                        </button>
                                    </div>
                                    <div class="song-card-info" style="padding: 15px;">
                                        <div class="song-card-title" style="font-weight: 600; font-size: 14px; color: #2c3e50; margin-bottom: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo htmlspecialchars($similarSong['title']); ?></div>
                                        <div class="song-card-artist" style="font-size: 12px; color: #7f8c8d; margin-bottom: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo htmlspecialchars($displayArtistName); ?></div>
                                        <div style="font-size: 11px; color: #95a5a6;">
                                            <?php echo number_format($similarSong['plays'] ?? 0); ?> plays
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Debug song data
            <?php 
                // Use same logic as download.php to get audio file
                $audio_file = $song['audio_file'] ?? $song['file_path'] ?? '';
                
                if (empty($audio_file)) {
                    $audio_file = 'uploads/audio/demo.mp3';
                }
                
                // Replace backslashes with forward slashes for consistency
                $audio_file = str_replace('\\', '/', $audio_file);
                
                // Build audio URL - use same pattern as download.php
                // Try to resolve the actual file path first
                $base_dir = realpath(__DIR__);
                $possible_paths = [
                    realpath($audio_file),  // If already absolute
                    realpath($base_dir . '/' . $audio_file),  // Relative to project root
                    realpath(__DIR__ . '/' . $audio_file),  // Relative to current file
                ...
                // Use stream.php if it exists, otherwise use direct file path
                if (file_exists(__DIR__ . '/api/stream.php')) {
                    $audioUrl = $baseUrl . 'api/stream.php?id=' . (int)$song['id'];
                } else {
                    // Use direct file path - normalize for URL
                    $audioUrl = $baseUrl . str_replace('\\', '/', $audio_file);
                }
                
                echo "console.log('Song data:', " . json_encode($song) . ");";
                echo "console.log('Audio file path:', '" . htmlspecialchars($audio_file) . "');";
                echo "console.log('Audio URL:', '" . htmlspecialchars($audioUrl) . "');";
            ?>
            
            // Initialize audio player
            const songId = <?php echo (int)$song['id']; ?>;
            const audioUrl = '<?php echo htmlspecialchars($audioUrl); ?>';
            ...
            audio.crossOrigin = 'anonymous'; // Allow CORS for metadata
            audio.src = audioUrl;
                    ...
                    if (audio.duration && !isNaN(audio.duration) && audio.duration > 0) {
                        ...
                        setTimeout(function() {
                            if (audio.duration && !isNaN(audio.duration) && audio.duration > 0) {
                                ...
                                console.warn('Duration still not available after metadata load');
                                // Try to get duration from database or file
                                const songDuration = <?php echo !empty($song['duration']) ? (int)$song['duration'] : 'null'; ?>;
            ...
            // Play/Pause button handler - must be inside DOMContentLoaded to access audio
            ...
            // Make updatePlayButton available globally
        ...
        // Progress bar clicking (must wait for DOM)
        document.addEventListener('DOMContentLoaded', function() {
        ...
        // Progress bar clicking (legacy - keep for compatibility)
        ...
        if (downloadBtn) {
            ...
            downloadBtn.addEventListener('click', function(e) {
                ...
                .catch(error => {
                    ...
                    const directDownloadUrl = downloadBtn.getAttribute('href');
