...
// Also handle case where artist name is passed directly (e.g., "admin")
$artist_name = $artist_param ? str_replace('-', ' ', urldecode($artist_param)) : '';
$artist_id = $_GET['id'] ?? null;

// If artist_name is empty but we have a slug, try to decode it
if (empty($artist_name) && !empty($artist_param)) {
    $artist_name = urldecode($artist_param);
}
...
<head>
    ...
    if (!empty($artist_data)) {
        ...
        $shareUrl = SITE_URL . '/artist/' . urlencode($artist_slug);
        $shareTitle = htmlspecialchars($artist_name . ' - Artist Profile');
        $shareDescription = !empty($artist_data['bio']) 
            ? htmlspecialchars(strip_tags(substr($artist_data['bio'], 0, 200))) 
            : htmlspecialchars('Listen to music by ' . $artist_name . ' on ' . SITE_NAME);
        
        // Get share image - use artist avatar, then cover image, then default
        $shareImage = '';
        if (!empty($artist_data['avatar'])) {
            $shareImage = asset_path($artist_data['avatar']);
        } elseif (!empty($artist_data['cover_image'])) {
            $shareImage = asset_path($artist_data['cover_image']);
        } else {
            $shareImage = defined('SITE_URL') ? SITE_URL . '/assets/images/default-avatar.svg' : '';
        }
    ?>
