        ...
        .then(response => {
            return response.text().then(text => {
                try {
                    ...
                    if (!response.ok) {
                        // If we have error details in JSON, use them
                        const errorMsg = data.error || 'Network response was not ok: ' + response.status;
                    ...
                    // If JSON parsing failed, show the raw response
                    throw new Error('Invalid JSON response: ' + text.substring(0, 200));
                }
            });
        })
