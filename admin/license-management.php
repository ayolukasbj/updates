            ...
            switch ($action) {
                case 'activate_license':
                    ...
                    $verify_data = [
                        ...
                        'domain' => $domain,
                        'ip' => $ip,
                        'version' => defined('APP_VERSION') ? APP_VERSION : (defined('SCRIPT_VERSION') ? SCRIPT_VERSION : '1.0')
                    ];
                ...
                case 'deactivate_license':
                    // Completely remove license key from settings and database
                    try {
                        // Remove from settings table
                        $removeStmt = $conn->prepare("DELETE FROM settings WHERE setting_key IN ('license_key', 'license_domain')");
                        $removeStmt->execute();
                        
                        // Also remove from licenses table if it exists (local copy)
                        try {
                            $checkTable = $conn->query("SHOW TABLES LIKE 'licenses'");
                            if ($checkTable->rowCount() > 0) {
                                $removeLicenseStmt = $conn->prepare("DELETE FROM licenses WHERE license_key = ?");
                                $removeLicenseStmt->execute([$current_license_key ?? '']);
                            }
                        } catch (Exception $e) {
                            // Table might not exist, ignore
                        }
                        
                        // Notify license server about deactivation (optional)
                        if (!empty($current_license_key)) {
                            ...
                            // Make API call to deactivate on server (if API endpoint exists)
                            try {
                                $api_url = rtrim($license_server_url, '/') . '/api/deactivate-license.php';
                                $ch = curl_init($api_url);
                                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                                curl_setopt($ch, CURLOPT_POST, true);
                        ...
                        $success = 'License deactivated successfully. The platform will now be blocked from updates and important features. Please activate a new license to continue.';
                        if (function_exists('logAdminActivity')) {
                            logAdminActivity($_SESSION['user_id'] ?? 0, 'deactivate_license', 'license', 0, "Deactivated license: " . substr($current_license_key ?? '', 0, 10) . "...");
                        }
                        
                        // Clear license key variable
                        $current_license_key = '';
                        
                        // Redirect to show license management page again
                        header('Location: license-management.php?deactivated=1');
                        exit;
                    } catch (Exception $e) {
...
// Fallback to config constant if database doesn't have it (only if not deactivated)
if (empty($current_license_key) && defined('LICENSE_KEY') && !empty(LICENSE_KEY)) {
    $current_license_key = LICENSE_KEY;
}

// Check if license was just deactivated
$is_deactivated = isset($_GET['deactivated']) && $_GET['deactivated'] == '1';
...
<div class="card" style="margin-bottom: 30px;">
    ...
    <div class="card-body">
        ...
        <div style="margin-top: 15px;">
            <form method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to deactivate this license? This will completely remove the license from the platform and block access to updates and important features.');">
...
<div class="card" style="margin-bottom: 30px;">
    ...
    <div class="card-body">
        <div style="padding: 20px; background: #fff3cd; border-radius: 6px; margin-bottom: 15px;">
            <h3 style="color: #856404; margin: 0 0 10px 0;">
                <i class="fas fa-info-circle"></i> No License Activated
            </h3>
            <p style="color: #856404; margin: 0;">
                Please activate a license key to use all platform features and receive updates.
            </p>
        </div>
