    ...
    if (empty($title) || empty($content)) {
        ...
        try {
            if ($edit_mode) {
                ...
                if ($is_published == 1) {
                    ...
                    $stmt = $conn->prepare("
                        UPDATE news SET 
                            ...
                            content = ?, excerpt = ?, co_author = ?, is_published = ?, featured = ?, author_id = ?
                        WHERE id = ?
                    ");
                    $stmt->execute([$title, $slug, $category, $image_path, $content, $excerpt, $co_author, $is_published, $featured, $_SESSION['user_id'], $news_id]);
                } else {
                    // Not publishing, just update without changing author_id
                    $stmt = $conn->prepare("
                        UPDATE news SET 
                            title = ?, slug = ?, category = ?, image = ?, 
                            content = ?, excerpt = ?, co_author = ?, is_published = ?, featured = ?
                        WHERE id = ?
                    ");
                    $stmt->execute([$title, $slug, $category, $image_path, $content, $excerpt, $co_author, $is_published, $featured, $news_id]);
                }
                ...
                // Insert
                $stmt = $conn->prepare("
                    INSERT INTO news (title, slug, category, image, content, excerpt, co_author, is_published, featured, author_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$title, $slug, $category, $image_path, $content, $excerpt, $co_author, $is_published, $featured, $_SESSION['user_id']]);
