...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($title)) {
            ...
            if (isset($_POST['file_path']) && !empty(trim($_POST['file_path']))) {
                $update_fields[] = 'file_path = ?';
                $update_values[] = trim($_POST['file_path']);
            }
            ...
            <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                ...
                <div style="margin-top: 15px;">
                    ...
                    <?php if (empty($song['file_path'])): ?>
                        <span class="badge badge-warning" style="margin-left: 10px;">Empty - File needs to be located</span>
                    <?php endif; ?>
                    <br>
                    <div style="display: flex; gap: 10px; margin-top: 5px; align-items: flex-start;">
                        <input type="text" name="file_path" id="file_path_input" class="form-control" 
                               value="<?php echo htmlspecialchars($song['file_path'] ?? ''); ?>" 
                               placeholder="e.g., uploads/audio/filename.mp3"
                               style="flex: 1;">
                        <button type="button" class="btn btn-info btn-sm" onclick="findFileBySize(<?php echo $song['id']; ?>, <?php echo $song['file_size']; ?>)">
                            <i class="fas fa-search"></i> Find File
                        </button>
                    </div>
                    <small class="text-muted" style="display: block; margin-top: 5px;">
                        Enter the file path manually or click "Find File" to search automatically by file size.
                    </small>
                    <?php if (!empty($song['file_path'])): ?>
                        <code style="background: #f3f4f6; padding: 8px; display: block; margin-top: 5px; border-radius: 4px; word-break: break-all; font-size: 12px;">
                            Current: <?php echo htmlspecialchars($song['file_path']); ?>
                        </code>
                    <?php endif; ?>
                </div>
            </div>
...
<script>
function findFileBySize(songId, fileSize) {
    if (!fileSize || fileSize <= 0) {
        alert('File size is not available. Cannot search for file.');
        return;
    }
    ...
    .then(data => {
        ...
        if (data.success && data.file_path) {
            document.getElementById('file_path_input').value = data.file_path;
            alert('File found! Path: ' + data.file_path + '\n\nClick "Save Changes" to update the database.');
        } else {
            alert('File not found. Please enter the file path manually.\n\nSearched in:\n- uploads/audio/\n- uploads/music/\n- music/\n- uploads/');
        }
    })
    .catch(error => {
        btn.disabled = false;
        btn.innerHTML = originalText;
        alert('Error searching for file: ' + error.message);
    });
}
</script>
