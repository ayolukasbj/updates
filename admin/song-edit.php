    ...
    if (empty($title)) {
        ...
        try {
            ...
            if ($checkCol->rowCount() == 0) {
                $conn->exec("ALTER TABLE songs ADD COLUMN share_excerpt TEXT NULL AFTER description");
            }
        } catch (Exception $e) {
        ...
        $stmt = $conn->prepare("
            UPDATE songs 
            SET title = ?, artist_id = ?, album_id = ?, lyrics = ?, share_excerpt = ?, is_featured = ?, is_explicit = ?
            WHERE id = ?
        ");
        
        if ($stmt->execute([$title, $artist_id, $album_id, $lyrics, $share_excerpt, $is_featured, $is_explicit, $song_id])) {
